...
export class CryptoChartComponent implements OnInit, OnDestroy {
  public chartOptions: ApexOptions = {
    ...
    chart: { type: 'line', height: 300 },  // ✅ Ensure `chart` is always defined
    xaxis: { type: 'datetime' },
    yaxis: { title: { text: 'Price (USD)' } },
    title: { text: 'BTC-USD Live Chart', align: 'center' },
    dataLabels: { enabled: false },
    stroke: { curve: 'smooth' },  // ✅ Ensure `stroke` is always defined
  ...
  ngOnInit(): void {
    this.priceSub = this.cryptoWsService.priceUpdates$.subscribe((update: PriceUpdate) => {
      ...
      if (btcPrice) {
        ...
        this.chartOptions = {
          ...this.chartOptions,
          chart: this.chartOptions.chart ?? { type: 'line', height: 300 }, // ✅ Ensure `chart` is always defined
          stroke: this.chartOptions.stroke ?? { curve: 'smooth' }, // ✅ Ensure `stroke` is always defined
