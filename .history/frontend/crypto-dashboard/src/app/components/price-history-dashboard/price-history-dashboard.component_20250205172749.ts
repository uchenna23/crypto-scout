...
export class PriceHistoryDashboardComponent implements OnInit {
  ...
  fetchPriceHistory(): void {
    const historicalDataRequests = this.coins.map((coin) =>
      this.http.get<any>(`https://api.coinbase.com/v2/prices/${coin}/historic?granularity=86400`)
    );

    forkJoin(historicalDataRequests).subscribe((responses) => {
      const datasets = responses.map((response, index) => {
        const data = response?.prices || response?.data?.prices; // ✅ Handle potential API structures
      ...
      const timeLabels = responses[0]?.prices?.map((entry: any) =>
        new Date(entry.time * 1000).toLocaleDateString()
      );
