...
export class PriceHistoryDashboardComponent implements OnInit {
  ...
  selectedRange = 'dtd'; // Default to Day-to-Day
  ...
  filterDataByRange(data: any[]): any[] {
    ...
    switch (this.selectedRange) {
      case 'dtd':
        return data.filter((entry) => new Date(entry[0] * 1000).toDateString() === now.toDateString());
      case 'mtd':
      ...
      case 'ytd':
        return data.filter((entry) => new Date(entry[0] * 1000).getFullYear() === now.getFullYear());
      default:
        return data;
    }
  }
