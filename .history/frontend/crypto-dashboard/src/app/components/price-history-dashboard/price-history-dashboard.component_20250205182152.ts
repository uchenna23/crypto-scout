    ...
    this.chartOptions = {
      ...
      scales: {
        x: {
          ...
          type: 'category' // Ensure x-axis is category type for date labels
        },
        y: {
          display: true,
          title: {
  ...
  filterDataByRange(data: any[]): any[] {
    ...
    switch (this.selectedRange) {
      case 'mtd':
        ...
        return data.filter((entry) => entry[0] * 1000 >= startOfMonth && entry[0] * 1000 <= now.getTime());
      case 'ytd':
        const startOfYear = new Date(now.getFullYear(), 0, 1).getTime();
        return data.filter((entry) => entry[0] * 1000 >= startOfYear && entry[0] * 1000 <= now.getTime());
      default:
  ...
  zoomToSelectedRange(): void {
    if (this.priceHistoryChart && this.priceHistoryChart.chart) {
      ...
      if (this.selectedRange === 'mtd' || this.selectedRange === 'ytd') {
        ...
        const startIndex = labels.findIndex((label: any) => new Date(label).getTime() >= startDate.getTime());
        const endIndex = labels.findIndex((label: any) => new Date(label).getTime() > now.getTime());
