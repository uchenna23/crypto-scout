...
public class CryptoWebSocketHandler extends TextWebSocketHandler {
    ...
    public CryptoWebSocketHandler() {
        connectToCoinbase();
        startBatchProcessor();
        startKeepAlive(); // ✅ Start heartbeat for long-lived connections
    }
            ...
            if (!messageBuffer.isEmpty() && clientSession != null && clientSession.isOpen()) {
                try {
                    ...
                    if (latestUpdate != null) {
                        ...
                        System.out.println("📩 Sent hourly update: " + latestUpdate);
                    }
                    messageBuffer.clear(); // ✅ Clear buffer after sending
                } catch (Exception e) {
                    System.err.println("❌ Error sending batch messages: " + e.getMessage());
                }
            }
        }, 0, 1, TimeUnit.HOURS); // ✅ Now updates every 1 hour
    }

    // 🔹 Keep WebSocket alive by sending a ping every 10 minutes
