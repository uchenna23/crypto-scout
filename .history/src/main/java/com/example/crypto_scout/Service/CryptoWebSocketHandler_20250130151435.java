...
public class CryptoWebSocketHandler extends TextWebSocketHandler {
    ...
    // Buffer for JSON strings (e.g. "{ \"BTC-USD\": 32000.12 }")
    ...
    // ✅ Keep track of the last time we sent each product update, so we can skip frequent updates
    ...
    // 🔹 Only update if price changes by more than 0.1%
    private static final double PRICE_CHANGE_THRESHOLD = 0.1;
    // 🔹 Skip updates if the last one for the same product was within N seconds
    private static final long MIN_UPDATE_INTERVAL_MS = 2000; // e.g. 2 seconds
        ...
        try {
            coinbaseWebSocket = new WebSocketClient(new URI("wss://advanced-trade-ws.coinbase.com")) {
                ...
                public void onOpen(ServerHandshake handshakedata) {
                    ...
                    // 🔹 SUBSCRIBE TO FEWER PRODUCT IDS 
                    //    If you only care about BTC-USD & ETH-USD, for example, reduce the list
                    Map<String, Object> subscribeMessage = Map.of(
                            "type", "subscribe",
                            "channel", "ticker",
                            "product_ids", List.of("BTC-USD", "ETH-USD") 
                            // Example: Only 2 products, not all.
                    );
                        ...
                        try {
                            ...
                            for (Object obj : eventsList) {
                                ...
                                for (Object tObj : tickersList) {
                                    ...
                                    // ✅ Step 1: Check if price changed enough
                                    if (!trackPriceChange(productId, newPrice)) {
                                        continue; // Skip if no significant change
                                    }

                                    // ✅ Step 2: Check if last update was too recent
                                    if (!rateLimitPassed(productId)) {
                                        continue; // Skip if we just updated
                                    }

                                    // ✅ If we pass both checks, buffer this update
    ...
    /**
     * Only broadcast if price moves more than PRICE_CHANGE_THRESHOLD%.
     * Ensures the first update for a product always qualifies (lastPrice < 0).
     */
    private boolean trackPriceChange(String productId, double newPrice) {
        double lastPrice = lastPrices.getOrDefault(productId, -1.0);

        if (lastPrice < 0.0) {
            // First update for this product, store & broadcast
    ...
    /**
     * Rate limit: ensure we don't buffer an update for the same product if we did so too recently.
     */
    private boolean rateLimitPassed(String productId) {
        long now = System.currentTimeMillis();
        long lastTime = lastUpdateTimestamps.getOrDefault(productId, 0L);

        // If enough time has passed, allow it
    ...
    /**
     * Cache the price in Redis for future reference, if desired.
     */
    ...
    /**
     * Periodically batches and sends updates to Angular via /topic/crypto
     */
    private void startBatchProcessor() {
        // Start after 5 seconds, repeat every 10 seconds
