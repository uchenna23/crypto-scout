...
public class CryptoWebSocketHandler extends TextWebSocketHandler {
    ...
    private int connectionLostTimeoutSeconds = 60; // Default timeout in seconds
    private volatile long lastPongTime = System.currentTimeMillis();

    public CryptoWebSocketHandler(SimpMessagingTemplate messagingTemplate) {
    ...
    /**
     * Allows external configuration of the connection lost timeout.
     */
    public void setConnectionLostTimeout(int timeoutSeconds) {
        this.connectionLostTimeoutSeconds = timeoutSeconds;
    }
    ...
    private void connectToCoinbase() {
        try {
            coinbaseWebSocket = new WebSocketClient(new URI("wss://advanced-trade-ws.coinbase.com")) {
                ...
                // Added to update the lastPongTime when a pong is received.
                @Override
    ...
    // Added: Monitor the pong responses and trigger reconnect if timeout is exceeded.
        ...
        scheduler.scheduleAtFixedRate(() -> {
            if (coinbaseWebSocket != null && coinbaseWebSocket.isOpen()) {
                ...
                if (elapsed > connectionLostTimeoutSeconds * 1000L) {
                    System.err.println("❌ No pong received within timeout (" + connectionLostTimeoutSeconds + "s). Reconnecting...");
    ...
    private void reconnectToCoinbase() {
        System.out.println("🔄 Reconnecting to Coinbase in 60 seconds...");
