...
public class CryptoWebSocketHandler extends TextWebSocketHandler {
            ...
            coinbaseWebSocket = new WebSocketClient(new URI("wss://advanced-trade-ws.coinbase.com")) {
                ...
                public void onMessage(String message) {
                    ...
                    messageProcessingExecutor.submit(() -> {
                        try {
                            // Deserialize JSON into a Map safely
                            ...
                            if (!(eventsObj instanceof List<?>)) return; // ✅ Ensure correct type
                            ...
                            for (Object obj : eventsList) {
                                if (obj instanceof Map) {
                                    @SuppressWarnings("unchecked") // ✅ Suppress warning safely
                            ...
                            for (Map<String, Object> event : events) {
                                // Safely extract "tickers"
                                Object tickersObj = event.get("tickers");
                                if (!(tickersObj instanceof List<?>)) continue; // ✅ Ensure correct type
    ...
    private void startBatchProcessor() {
        scheduler.scheduleAtFixedRate(() -> {
            System.out.println("🕒 Batch Processor Running..."); // Log each execution
            ...
            if (!messageBuffer.isEmpty()) {
                try {
                    ...
                    if (!batchedUpdates.isEmpty()) {
                        ...
                        System.out.println("🔄 Preparing to send update: " + json); // Debugging line
                        messagingTemplate.convertAndSend("/topic/crypto", json);
                        System.out.println("📩 Sent to WebSocket Clients: " + json); // Debugging line
                    } else {
