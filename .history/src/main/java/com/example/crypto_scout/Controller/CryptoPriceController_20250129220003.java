...
public class CryptoPriceController {
    ...
    @GetMapping("/bulk")
public ResponseEntity<Map<String, Object>> getBulkPrices(@RequestParam String currencyPairs) {
    // Split the comma-separated string into a list
    List<String> currencyPairList = Arrays.asList(currencyPairs.split(","));

    // Validate each currency pair
    for (String pair : currencyPairList) {
        if (!pair.matches("^[A-Za-z0-9-]+$")) {
            return ResponseEntity.badRequest().body(Map.of("error", "Invalid currency pair: " + pair));
        }
    }

    Map<String, Object> response = cryptoPriceService.getMultipleCryptoPrices(currencyPairList);
