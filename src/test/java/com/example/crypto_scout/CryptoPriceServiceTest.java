...
class CryptoPriceServiceTest {
    ...
    void testGetMultipleCryptoPrices() {
        // Arrange: stub for each currency pair
        Map<String, Object> coinbaseA = Map.of("data", Map.of("base", "A", "currency", "USD", "amount", "1"));
        Map<String, Object> coinbaseB = Map.of("data", Map.of("base", "B", "currency", "USD", "amount", "2"));
        // Stub spot price calls
        when(restTemplate.getForObject(contains("prices/A-USD/spot"), eq(Map.class))).thenReturn(coinbaseA);
        when(restTemplate.getForObject(contains("prices/B-USD/spot"), eq(Map.class))).thenReturn(coinbaseB);
        // Stub historical data calls (no chart data)
        when(restTemplate.getForObject(contains("market_chart"), eq(Map.class))).thenReturn(Map.of());

        // Act
        Map<String, Object> result = service.getMultipleCryptoPrices(List.of("A-USD", "B-USD"));

        // Assert: two entries with correct amounts and default chart message
        assertEquals(2, result.size());
        Map<String, Object> resA = (Map<String, Object>) result.get("A-USD");
        Map<String, Object> resB = (Map<String, Object>) result.get("B-USD");
        assertEquals("1", resA.get("amount"));
        assertEquals("Historical data not available", resA.get("chart"));
        assertEquals("2", resB.get("amount"));
        assertEquals("Historical data not available", resB.get("chart"));
    }
